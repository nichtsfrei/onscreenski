...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            xdg_dirs: xdg::BaseDirectories::with_prefix("onscreenski").unwrap(),
...
impl AppConfig {
    ...
    fn get_config_file_content(&self, file_name: &str, fallback: &str) -> String {
        match self.xdg_dirs.find_config_file(file_name) {
            Some(css_path) => fs::read_to_string(css_path).unwrap(),
    ...
    pub fn get_layout_file_content(&self) -> String {
        ...
        self.get_config_file_content("layout.toml", &LayoutAssets::default_layout_str())
    }
}
